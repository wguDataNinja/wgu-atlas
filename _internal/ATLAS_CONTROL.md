...
| Courses (course-page enrichment) | **Active — Session 2 prototype complete** — sandboxed cohort preview surface built and verified; all 10 design cohort pages live at `/proto/course-preview/[code]` | Review cohort previews; produce shape-disposition / display-policy artifact to resolve open questions before production implementation | multi-variant collapse policy, prereq-context gap, prereq–description redundancy unresolved | review content map (`_internal/course_pages/content_maps/session2_cohort_preview.txt`), then write policy artifact |
